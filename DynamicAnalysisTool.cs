...
public class DynamicAnalysisTool
{
    ...
    {
        Console.WriteLine($"Mode: Cleaning '{Path.GetFileName(dllPath)}' for analysis (Invalidate, Reorder & Rename All)...");
        var outputPath = Path.Combine(Path.GetDirectoryName(dllPath), $"{Path.GetFileNameWithoutExtension(dllPath)}_final_analysis.dll");
        ...
        var readerParams = new ReaderParameters { ReadWrite = true, AssemblyResolver = resolver, ReadingMode = ReadingMode.Immediate };
